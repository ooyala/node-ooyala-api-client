...
function OoyalaApiClient( apiKey, apiSecret ) {
	...
	this._buildPromise = function( method, apiPath, params, body ) {
		...
		params['signature'] = this._generateSignature(params, method.toUpperCase(), apiPath, body );

		request[method.toLowerCase()]( this.urlRoot + apiPath )
			.accept('application/json').set('X-API-KEY', this.apiKey ).set('HTTP_X_API_KEY', this.apiKey)
			.query(params)
			.send(body)
